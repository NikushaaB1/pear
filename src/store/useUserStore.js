...
export const useUserStore = create(
  persist(
    (set, get) => ({
      ...
      setUser: (user, role, modelId) => set({ user, role, modelId }),
      clearUser: () => set({ user: null, role: null, modelId: null }),
      ...
      resolveModel: (id) => {
        ...
        const { role, modelId, user } = get()
        if (role === 'model' && modelId === id && user) {
          return get().ensureModelFromProfile({
            uid: user.uid,
            email: user.email,
