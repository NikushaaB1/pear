...
import { Users, Upload, Megaphone, Trophy, ArrowRight, Sparkles } from 'lucide-react'
...
export default function Dashboard() {
  ...
  const modelsRoute = role === 'admin' ? '/models' : modelId ? `/models/${modelId}` : '/dashboard'
  const uploadsRoute = role === 'admin' ? '/uploads' : modelId ? `/models/${modelId}` : '/dashboard'

  const dashboardCards = [
    { title: 'მოდელები', description: role === 'admin' ? `${models.length} მოდელი` : 'შენი პროფილი', icon: Users, to: modelsRoute },
    { title: 'ატვირთვები', description: role === 'admin' ? 'ყველა ფოტო' : 'შენი გალერეა', icon: Upload, to: uploadsRoute },
